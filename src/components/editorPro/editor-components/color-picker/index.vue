...
<template>
  <div class="color-picker">
    ...
    <!-- 绘本 / 预设色卡 -->
    <div v-if="mode === '色卡' && displaySwatches.length" class="cp__swatches">
      <button
        v-for="(sw, idx) in displaySwatches"
        :key="idx"
        type="button"
        class="cp__swatch"
        :title="sw.label"
        :style="{ background: sw.hex }"
        @click="onSwatchPick(sw.hex)"
      />
    </div>
    ...
    <div v-show="mode !== '色卡'" ref="elSliderHux" class="cp__slider cp__slider-hux">
      <div class="cps__track">
        <div ref="elSliderHuxPointer" class="cpst__pointer"></div>
      </div>
    </div>

    <div v-show="mode !== '色卡'" ref="elSliderAlpha" class="cp__slider cp__slider-alpha">
...
import { registerMoveableElement } from './utils/moveable.js';
import { hexA2HSLA, HSLA2HexA, hex2RGB, RGB2HSL, hexA2RGBA, RGBA2HexA } from './utils/color.js';
import { toGradientString, parseBackgroundValue, toolTip } from './utils/helper.js';
...
const props = defineProps({
  ...
  /** 色卡模式：{ label: string, hex: string }，hex 可为 #RRGGBB */
  swatches: {
    type: Array,
    default: () => [],
  },
...
/** 打开取色器时：有「色卡」且当前是 hex 纯色 → 默认停在色卡，不跳到纯色 */
function getInitialPickerMode() {
  const base = parseBackgroundValue(props.value);
  if (props.modes.includes('色卡') && base === '纯色') {
    return '色卡';
  }
  return base;
}

const mode = ref(getInitialPickerMode());
const angle = ref(90);
const gradients = ref([]);
...
const displaySwatches = computed(() => {
  const list = props.swatches || [];
  return list.filter((s) => s && s.hex);
});
...
watch(
  () => props.value,
  (value) => {
    const valueMode = parseBackgroundValue(value);
    let targetMode = mode.value;

    // hex 更新时不要把「色卡」强行改回「纯色」（否则会跳 Tab）
    if (valueMode === '纯色') {
      if (props.modes.includes('色卡') && (mode.value === '色卡' || mode.value === '渐变')) {
        targetMode = '色卡';
      } else {
        targetMode = '纯色';
      }
    } else if (valueMode !== mode.value) {
...
function onChangeHSLA(newHsla) {
  ...
  let value;
  if (mode.value === '纯色' || mode.value === '色卡') {
    value = hexA;
...
/** 色卡点击：同步到画布使用纯色（与 attributeColor 的 colorChange 一致） */
function onSwatchPick(hex) {
  let h = String(hex || '').trim();
  if (!h) return;
  if (!h.startsWith('#')) h = `#${h}`;
  const hexA = h.length === 7 ? `${h}ff` : h.length >= 9 ? h : `${h}ff`;
  setColor(hexA);
  record.color = hexA;

...
const onModalOk = async () => {
  ...
  const ops = cloneDeep(waterMarkState);
  ops.isRotate = !!ops.isRotate; // 转为对应类型  后续再统一处理类型
  canvasEditor && canvasEditor.drawWaterMark && (await canvasEditor.drawWaterMark(ops));
  showWaterMadal.value = false;
  // onMadalCancel();
};
...
const changeFontFamily = (fontName) => {
  if (!fontName) return;
  canvasEditor && canvasEditor.loadFont && canvasEditor.loadFont(fontName);
};

...
<style scoped>
@import url('@/assets/editorpro/css/vue-color-gradient-picker/dist/index.css');
...
/* 外层 .attr-item 里的 ivu 相关样式展开 */
.attr-item :deep(.ivu-slider-wrap) {
  margin: 13px 0;
}

.attr-item :deep(.ivu-radio-group-button .ivu-radio-wrapper) {
  width: 48px;
  line-height: 40px;
  text-align: center;
}

.attr-item :deep(.ivu-radio-group-button .ivu-radio-wrapper svg) {
  vertical-align: baseline;
}

.attr-item :deep(.ivu-btn-group-large > .ivu-btn) {
  font-size: 24px;
}

.attr-item :deep(.ivu-radio-group-button.ivu-radio-group-large .ivu-radio-wrapper) {
  font-size: 24px;
}
